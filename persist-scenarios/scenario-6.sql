...
update GMS4.sms_customer_contacts
    ...
    where contract_id in (select cc.contract_id from GMS4.sms_customer_contacts cc where cc.value like 'mansir-006%@mailinator.com' and cc.contact_type_id = 1)
;    
 -- Update PP service:
update GMS4.sms_customer_services
    ...
    where contract_id in (
        select cc.contract_id from GMS4.sms_customer_contacts cc 
            where cc.value in like 'mansir-006%@mailinator.com' 
...
;
 -- Update SC service:
update GMS4.sms_customer_services
    set service_status_id = 3, last_updated = current timestamp - 6 hours
    where contract_id in (
        select cc.contract_id from GMS4.sms_customer_contacts cc 
            where cc.value in ( 'mansir-0006@mailinator.com' ) 
                and cc.contact_type_id = 1
            )
